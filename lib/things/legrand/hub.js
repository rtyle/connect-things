...
module.exports = class Hub extends EventEmitter {
	...
	scatter(chunk) {
		...
		while (-1 !== (end = chunk.indexOf(0, begin))) {
			...
			try {
				...
				this.logger.error(error, 'JSON.parse', json)
			}
			begin = end + 1
		}
		return begin
	...
	constructor(logger, port = defaultPort, host = defaultHost) {
		super()
		...
		this.logger.info('socket connecting to', host + ':' + port)
		this.socket
			.on('error', (error) => {
				this.logger.error('socket', error.name + ':', error.message)
			})
			.on('timeout', () => {
			...
			.on('close', () => {
				...
				setTimeout(() => {
					logger.warn('socket reconnecting to', host + ':' + port)
	...
	request(action, once = null) {
		...
		this.logger.debug('<\t', JSON.stringify(action))
