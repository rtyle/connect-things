...
class Device {
	...
	changed(thing, serviceType, key, value) {
		...
		if (undefined === this.connector.callbackUrls || undefined === this.connector.callbackAuthentication) {
			...
			this.logger.info('<', this.thing.friendlyName, this.connector.callbackUrls, this.connector.callbackAuthentication, stateDevice)
	...
	constructor(logger, clientId, clientSecret, constructableDevice) {
		...
		super
			...
			.callbackAccessHandler((access_token, callbackAuthentication, callbackUrls, body) => {
				this.logger.error('>', 'callbackAccess', callbackAuthentication, callbackUrls, body)
				// TODO persist these things and recover them upon construction
