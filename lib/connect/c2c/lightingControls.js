...
curl -H 'Content-Type: application/json' -X POST -d '{
	...
	"authentication": {
		"tokenType"		: "Bearer",
		"token"			: "c3404b0f-df4c-3a3a-b89b-260a35eb5aeb"
	}
}' http://localhost:8081/c2c/resource
...
class Device {
	...
	discoveryRequest(request) {
		if (this.hasOwnProperty('handler')) {
			const discoveryDevice = new DiscoveryDevice(this.thing.uuid, this.thing.friendlyName, this.handler)
	...
	discoveryResponse(response) {
		if (this.hasOwnProperty('handler')) {
			const discoveryDevice = response.addDevice(this.thing.uuid, this.thing.friendlyName, this.handler)
			this.discovery(discoveryDevice)
		}
	}
	...
	stateRefresh(stateDevice) {
		if (this.hasOwnProperty('handler')) {
			...
			this.stateRefreshHealthCheck(stateDevice, 'offline')
		}
	}
	...
	command(commands, response) {
		if (this.hasOwnProperty('handler')) {
			...
			this.stateRefreshHealthCheck(response.addDevice(this.thing.uuid), 'offline')
		}
	}
	...
	changed(thing, serviceType, key, value) {
		...
		if (undefined === this.connector.callbackUrls || undefined === this.connector.callbackAuthentication) {
			...
			const request = new StateUpdateRequest(this.connector.clientId, this.connector.clientSecret)
			const stateDevice = new StateDevice(thing.uuid)
			switch (serviceType) {
			...
			default:
				this.stateRefreshHealthCheck(stateDevice, 'offline')
				logger.error('unexpected service type', serviceType)
			}
			this.logger.info('<', this.thing.friendlyName, stateDevice)
			request.updateState(this.connector.callbackUrls, this.connector.callbackAuthentication, [stateDevice],
				(callbackAuthentication) => {
					this.connector.callbackAuthenticationRefresh(callbackAuthentication)
				}
			)
		}
	}
...
module.exports = class LightingControls extends SchemaConnector {
	...
	constructor(logger, clientId, clientSecret, constructableDevice) {
		...
		this.callbackAuthenticationFile = `${path}/callbackAuthentication.${clientId}.json`
		try {
			this.callbackAuthentication = JSON.parse(fs.readFileSync(this.callbackAuthenticationFile))
		} catch (e) {
			logger.debug(e)
		}
		const callbackUrlsFile = `${path}/callbackUrls.${clientId}.json`
		try {
			this.callbackUrls = JSON.parse(fs.readFileSync(callbackUrlsFile))
		} catch (e) {
			logger.debug(e)
		}
		...
		super
			...
			.callbackAccessHandler((access_token, callbackAuthentication, callbackUrls, body) => {
				...
				fs.writeFileSync(callbackUrlsFile, JSON.stringify(callbackUrls))
			})
