...
class Device extends EventEmitter {
	...
	set(serviceType, key, value) {
		if (this[key] !== value) {
			this[key] = value
			this.emit('change', serviceType, key, value)
		}
	}
	setOwn(key, value) {
		switch (key) {
...
module.exports = class Factory extends EventEmitter {
	...
	reconcile(key, value) {
		if (this.map.has(key)) {
			...
			this.emit('new', device.deviceType, device)
			device.on('change', (serviceType, key, value) => {
				this.emit('change', device, serviceType, key, value)
			})
		}
	}
}
